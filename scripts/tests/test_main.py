...
def test_ffmpeg():
    in_filepath = os.path.abspath('./scripts/tests/resources/BigBuckBunny_320x180.mp4')
    out_filepath = os.path.abspath('./scripts/tests/resources/BigBuckBunny_320x180_output.avi')
    # ffmpeg.input(in_filepath).output(out_filepath, vcodec='copy', acodec='copy').overwrite_output().run()
    ffmpeg.input(in_filepath).output(out_filepath, acodec='copy', vcodec='copy').overwrite_output().run()
